...
export const calculateNumberOfSolvedSolution = (
  solutions: SolutionResponseData[]
) => {
  const solvedSolutions = solutions.filter(
...
export const calculateNumberOfVolunteering = (user: UserResponseData) => {
  return (
    user.pendingVolunteeredSolutions.length + user.volunteeredSolutions.length
  );
};
