...
export const SingleUserPost: React.FC<SingleUserPostProps> = ({
  ...
  const [isManageSolutionOpen, setIsManageSolutionOpen] = useState(false);
  ...
  useEffect(() => {
    if (echoBoardExtended) {
      setIsDisabled(
        echoBoardExtended.echoBoardSolutions === null ||
          echoBoardExtended.echoBoardSolutions.length === 0
      );
    }
  }, [echoBoardExtended]);
  ...
  const handleCloseManageSolutionsWindow = () => {
        ...
        <>
          ...
          >
            <Box sx={{ display: "flex", justifyContent: "space-between" }}>
              <Typography variant="h5" component="div" style={{ margin: "0" }}>
                {echoBoard.title}
              </Typography>
            ...
            <Box>
              {echoBoard.anonymous && (
                <Chip label="Posted anonymously" size="small" />
              )}
            </Box>
            ...
            <Box>
              {contentVisible ? (
                <Button size="small" onClick={hideContent}>
                  Hide Content
                </Button>
              ) : (
                <Button size="small" onClick={showContent}>
                  Show Content
                </Button>
              )}
            </Box>
            ...
            >
              ...
              >
                <Upvote
                  upvote={echoBoardExtended.upvote}
                  echoBoardId={echoBoardExtended.id}
                />
                ...
                >
                  <ModeCommentIcon />{" "}
                  {echoBoardExtended.echoBoardComments.length || 0}
                </Button>
                <Button
                  size="small"
                  onClick={() => {
              ...
              >
                {echoBoardExtended.echoBoardSolutions.length === 1 ? (
                  <Typography>
                    {echoBoardExtended.echoBoardSolutions?.length} solution
                    suggested
                  </Typography>
                ) : (
                  <Typography>
                    {echoBoardExtended.echoBoardSolutions?.length} solutions
                    suggested
                  </Typography>
                )}
                <Button
          ...
          <ManageSolutionsWindow
            open={isManageSolutionOpen}
            onClose={handleCloseManageSolutionsWindow}
