...
export const VolunteersInfo: React.FC<VolunteerInfoProps> = ({ solution }) => {
  ...
  const {
    data: solutionVolunteers,
    isLoading,
    isError,
  } = useQuery<SolutionVolunteersResponseData>(
  ...
  useEffect(() => {
    if (solutionVolunteers) {
      setIsDisabled(
        solutionVolunteers.pendingVolunteers === null ||
          (solutionVolunteers.pendingVolunteers?.length === 0 &&
        ...
        {solution.status === "VOLUNTEERS_REQUIRED" && (
          ...
          >
            {solutionVolunteers ? (
              <Typography>
                Pending volunteers:{" "}
                {solutionVolunteers.pendingVolunteers
                  ? solutionVolunteers.pendingVolunteers?.length
                  : 0}
                , Accepted volunteers:{" "}
                {solutionVolunteers.volunteers
                  ? solutionVolunteers.volunteers.length
                  : 0}
              </Typography>
            ) : (
